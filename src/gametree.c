 ...
 * gametree.c
 *
 * Code that allows the AI to play the game, and build its gametrees.
 */
...
char* build_gt_minimax() {
	...
	int target = -(INF);
	for(ll_node_t* curr = ROOT->children->head; curr != NULL; curr = curr->next) {
	    gt_node* node = ((gt_node *) curr->data);
	    target = max(target, node->utility);
	}
	
	for(ll_node_t* curr = ROOT->children->head; curr != NULL; curr = curr->next) {
	    gt_node* node = ((gt_node *) curr->data);
	    if(target == node->utility) {
	        return node->move;
	    }
	}
	
	// If we get here we failed to find our target.
	printf("FAILED TO FIND A MOVE\n\n");
	return NULL;
}
...
char* build_gt_abpruning() {
    ...
    EXPANDED = 0;
    // Builds the tree and does alpha-beta pruning and minimax at the same time.
                ...
                for(int dir = 1; dir <= DIRECTIONS; dir++) {
                    ...
                    && !contains_state(root, child->state)) 
                    {
                        ...
                        if(root->state->currTurn == root->state->myTurn) { // MAX
                            ...
                            root->utility = max(root->utility, res);
                        } else { // MIN
